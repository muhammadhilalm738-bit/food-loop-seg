...
import { Heart, MapPin, Package, Scale, Clock, Check, Loader2, Users, Inbox } from 'lucide-react';
import { supabase } from '@/lib/supabase';
import type { Listing } from '@/types';

interface NGOFeedProps {
  listings: Listing[];
  loading: boolean;
  onClaim: () => void;
}

export default function NGOFeed({ listings, loading, onClaim }: NGOFeedProps) {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-stone-900">NGO Feed</h1>
            <p className="text-stone-600 text-sm">Edible food available for distribution</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-4 text-sm">
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-forest-100 text-forest-700 font-medium">
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center">
          ...
          <h3 className="font-semibold text-stone-700 mb-1">No listings available</h3>
          <p className="text-sm text-stone-500">
            New edible food listings will appear here as donors post them.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {availableListings.map((listing) => (
            <div key={listing.id} className="card p-5 border-forest-200 hover:border-forest-400 hover:shadow-md transition-all duration-200">
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-forest-100 text-forest-700 text-xs font-semibold">
              ...
              {showNameInput === listing.id ? (
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <button
                    ...
                    className="btn-primary bg-forest-600 hover:bg-forest-700 flex items-center justify-center gap-2"
                  >
                ...
                <button
                  ...
                  className="btn-primary bg-forest-600 hover:bg-forest-700 w-full flex items-center justify-center gap-2"
                >
